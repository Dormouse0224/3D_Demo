...
{
	path path_content = CPathMgr::GetContentDir();
	...
	for (UINT i = 0; i < m_vecContainer.size(); ++i)
	{
		for (UINT j = 0; j < m_vecContainer[i].vecMtrl.size(); ++j)
		{
			...
			for (size_t k = 0; k < vecPath.size(); ++k)
			{
				...
				CAssetMgr::GetInst()->Load<CTexture>(path_dest);
...
void CFBXLoader::CreateMaterial()
{
	...
	for (UINT i = 0; i < m_vecContainer.size(); ++i)
	{
		for (UINT j = 0; j < m_vecContainer[i].vecMtrl.size(); ++j)
		{
			...
			AssetPtr<CMaterial> pMaterial = CAssetMgr::GetInst()->FindAsset<CMaterial>(strPath);
			if (nullptr != pMaterial)
				continue;

			pMaterial = new CMaterial;

			pMaterial->SetShader(CAssetMgr::GetInst()->FindAsset<CGraphicShader>(L"EA_DeferredShader"));

			wstring strTexKey = m_vecContainer[i].vecMtrl[j].strDiff;
            AssetPtr<CTexture> pTex = CAssetMgr::GetInst()->FindAsset<CTexture>(strTexKey);
			if (NULL != pTex)
			...
			CAssetMgr::GetInst()->AddAsset(strPath, pMaterial.Get());
			pMaterial->Save(strMtrlName);
		}
	}
}
